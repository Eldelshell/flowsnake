...
import Acceleration from './converters/acceleration/Acceleration';

/**
 * Starts convertion process.
 * @param  {Number|BigDecimal|Object<Unit>} value - the value to convert. It can be a Decimal.js BigDecimal object
 * or a JavaScript primitive number.
 * @return {Function} a new Convert instance.
 */
const Convert = function(value) {
    if(value.value){
        // We have a unit object.
...
/**
 * Obtains a new Unit with the with a value.
 * @param  {String} unitId - the unit id.
 * @return {Object<Unit>} the unit with the given value.
 * @throws {Error} when something goes wrong.
 */
Convert.prototype.as = function(unitId) {
    if(this.descriptor) {
        const unit = this.descriptor.getUnitById(unitId);
        return this.descriptor.of(this.value, unit);
    }
    ...
    for (const k in this.descriptors) {
        ...
        const descriptor = this.descriptors[k];
        try {
            const unit = descriptor.getUnitById(unitId);
    ...
    throw new Error(`No descriptor found that matches the given unit ${unitId}`);
};
