...
export default class Unit {
    ...
    to(other){
        ...
        // Unit uses a matching table, so we obtain the value for the index of this unit.
        if(_.isArray(this.values) && other.values && _.isArray(other.values)){
            return other.values[this.value];
        }

        // We have a direct rate converter, so we use the formula.
        if(this.converter){
    ...
    static withPrefixes(base, prefixes) {
        ...
        prefixes.forEach((prefix) => {
            const name = prefix.name + base.name.toLowerCase();
            const u = new Unit({
