    ...
    to(other){
        ...
        if(other.system && this.system){
            if(other.system.name !== this.system.name && this.system.ratios && this.system.ratios[other.system.name]){
                ...
                if(_.isFinite(systemRate)){
                    return Decimal.mul(this.value, this.converter).mul(systemRate).div(other.converter);
                }

                if(_.isFunction(systemRate)){
                    return Decimal.div(systemRate(this.value), other.converter);
                }

                throw new Error('Invalid system rate converter. Expected a number or a function');
            }
        }
        ...
        if(this.converter){
            return Decimal.mul(this.value, this.converter).div(other.converter);
        }
        ...
        // Converters can also be numbers, for example, when we convert between Sound units
        if(_.isFinite(converter)){
            return Decimal.div(this.value, converter);
        }
